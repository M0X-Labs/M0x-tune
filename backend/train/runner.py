...
# IMPORTANT: invalidate_stale_compiled_cache() must run BEFORE `import ` below --
# 's compiled-trainer cache (_compiled_cache/) is read/populated as a side
# effect of importing /trl, so checking for staleness has to happen first. This is
...
# module-level `import ` (a few lines below, via `from backend.train.engine import
...
# would execute engine.py's own top-level `import ` as a side effect of the import
# statement itself -- cache_guard.py is deliberately kept free of any -touching
