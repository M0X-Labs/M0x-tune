...
# what 's own official installer does (see Get-TorchIndexUrl / "install PyTorch
# first" in /install.ps1): pointing at the right https://download.pytorch.org/whl/cuXXX
...
COMMON_PACKAGES = [
    ...
    # safe to pin directly. Version range matches what 's own pyproject.toml
    ...
    # `[windows]` (see /pyproject.toml) pins a *loose*,
    ...
    # `[huggingface]` pulls transformers/trl/peft/accelerate/datasets/etc.
    # ('s own tested-compatible version ranges) WITHOUT requiring xformers at
    # all --  transparently falls back to PyTorch's built-in SDPA attention
    # when xformers isn't importable, so training still works correctly, just without
    # that one optional attention-kernel speedup.
    #
    # IMPORTANT: pinned to an exact release, NOT `git+...@main`. This is the single
    # biggest source of "crashes with a different error on every device": installing
    # off a floating branch means every fresh install (new device, rebuilt Colab/Kaggle
    # session, re-run weeks later) can silently resolve to a different  commit,
    # which drags in a different trl/transformers/peft/accelerate combo each time.
    # Pinning an exact version -- and its known-compatible transitive versions below,
